...
type User struct {
	CommonAttributes
	Active            bool              `json:"active,omitempty"`            //Active informs as to whether this User record is currently live in the system
	Addresses         []Address         `json:"addresses,omitempty"`         //Addresses is the  physical mailing address for this User, as described in (address Element). Canonical Type Values of work, home, and other. The value attribute is a complex type with the following sub-attributes.
	DisplayName       string            `json:"displayName,omitempty"`       //DisplayName is the name of the User, suitable for display to end-users. The name SHOULD be the full name of the User being described if known
	Emails            []Email           `json:"emails,omitempty"`            //Emails are E-mail addresses for the user. The value SHOULD be canonicalized by the Service Provider, e.g. bjensen@example.com instead of bjensen@EXAMPLE.COM. Canonical Type values of work, home, and other.
	Entitlements      []Entitlement     `json:"entitlements,omitempty"`      //Entitlements is a collection of entitlements
	...
	Photos            []Photo           `json:"photos,omitempty"`            //Photos are URLs of photos of the User.
	ProfileURL        string            `json:"profileUrl,omitempty"`        //Profile URL is a fully qualified URL to a page representing the User's online profile
	PreferredLanguage string            `json:"preferredLanguage,omitempty"` //PreferredLanguage indicates the User's preferred written or spoken language.  Generally used for selecting a localized User interface. e.g., 'en_US' specifies the language English and country US.
	Roles             []Role            `json:"roles,omitempty"`             //Roles are a list of roles for the User that collectively represent who the User is; e.g., 'Student', 'Faculty'.
	Timezone          string            `json:"timezone,omitempty"`          //Timezone is the User's time zone in the 'Olson' timezone database format; e.g.,'America/Los_Angeles'
	Title             string            `json:"title,omitempty"`             //Title is the user's title, such as "Vice President.
	...
	UserType          string            `json:"userType,omitempty"`          //UserType is used to identify the organization to user relationship. Typical values used might be 'Contractor', 'Employee', 'Intern', 'Temp', 'External', and 'Unknown' but any value may be used.
	X509Certificates  []X509Certificate `json:"x509Certificates,omitempty"`  //X509Certificates is list of certificates issued to the User.
}

//Address is a street and country based addess for the identity
type Address struct {
	Multivalued
	Country       string `json:"country,omitempty"`       //Country is the contry location of the address
	Formatted     string `json:"formatted,omitempty"`     //Formatted is the full mailing address, formatted for display or use with a mailing label. This attribute MAY contain newlines.")
	Locality      string `json:"locality,omitempty"`      //Locality is the city or locality component.
	PostalCode    string `json:"postalCode,omitempty"`    //PostalCode is the zipcode or postal code component.
	Region        string `json:"region,omitempty"`        //Region is the state or region component.
	StreetAddress string `json:"streetAddress,omitempty"` //StreetAddress is the full street address component, which may include house number, street name, PO BOX, and multi-line extended street address information. This attribute MAY contain newlines.")
}
...
type Name struct {
	Formatted       string `json:"formatted,omitempty"`       //Formatted is the full name, including all middle names, titles, and suffixes as appropriate, formatted for display (e.g. Ms. Barbara J Jensen, III.)
	FamilyName      string `json:"familyName,omitempty"`      //FamilyName is the family name of the User, or Last Name in most Western languages (e.g. Jensen given the full name Ms. Barbara J Jensen, III.)
	GivenName       string `json:"givenName,omitempty"`       //GiveName is the given name of the User, or First Name in most Western languages (e.g. Barbara given the full name Ms. Barbara J Jensen, III.)
	MiddleName      string `json:"middleName,omitempty"`      //MiddleName is the middle name(s) of the User (e.g. Robert given the full name Ms. Barbara J Jensen, III.).
